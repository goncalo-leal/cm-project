...
while True:
    ...
    if info_interval == 10:
        if board["gateway"]:
            _thread.start_new_thread(
                tcp_syn, (board["mac"], board["gateway"],))
        info_interval = 0
    else:
        info_interval += 1
    ...
    if packet:
        ...
        if data[0] == 0x6 and not exist_in_buffer([(0, 0x7), [3, mac], (4, data[3])]):
            _thread.start_new_thread(
                arp_response, (mac, data[3], board["name"],))
        ...
        # ack
        elif data[0] == 0x3 and data[4] == mac and not exist_in_buffer([(0, 0x4), (3, mac), (4, data[3]), (5, data[5]+1), (6, json.dumps(info_to_send))]):
            ...
            if syn:
                _thread.start_new_thread(
                    tcp_ack, (mac, data[3], data[5], json.dumps(info_to_send),)
                )
